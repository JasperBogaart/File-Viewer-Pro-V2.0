...
# ---------------------------------------------------------------------------
# Screensaver / display-sleep onderdrukking (Windows)
# ---------------------------------------------------------------------------
import ctypes
import sys
...
def _keep_display_awake():
    """Vertel Windows dat de display actief moet blijven."""
...
def _allow_display_sleep():
    """Herstel de normale Windows-slaapstand bij afsluiten."""
    if sys.platform == "win32":
        try:
...
class FullscreenViewer(tk.Toplevel):
    def __init__(self, master: tk.Tk, slides: list[Slide], on_exit):
        ...
        self._awake_after_id: Optional[str] = None
    ...
    # ------------------------------------------------------------------
    # Screensaver onderdrukking
    # ------------------------------------------------------------------
    _AWAKE_INTERVAL_MS = 30_000  # elke 30 seconden

    def _start_awake_loop(self):
        _keep_display_awake()
        self._awake_tick()

    def _awake_tick(self):
        _keep_display_awake()
        self._awake_after_id = self.after(self._AWAKE_INTERVAL_MS, self._awake_tick)

    def _stop_awake_loop(self):
        if self._awake_after_id is not None:
            try:
                self.after_cancel(self._awake_after_id)
            except Exception:
                pass
            self._awake_after_id = None
        _allow_display_sleep()
